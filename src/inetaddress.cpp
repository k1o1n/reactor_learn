...
namespace adachi::network {
    ...
    bool INetAddress::SetIP(const std::string& ip) {
        if (addr_.ss_family == AF_INET6) {
            return inet_pton(addr_.ss_family, ip.c_str(), &(reinterpret_cast<sockaddr_in6*>(&addr_))->sin6_addr) == 1;
        }
        else {
            return inet_pton(addr_.ss_family, ip.c_str(), &(reinterpret_cast<sockaddr_in*>(&addr_))->sin_addr) == 1;
        }
    }
    void INetAddress::SetPort(const in_port_t& port) {
        if (addr_.ss_family == AF_INET6) {
            (reinterpret_cast<sockaddr_in6*>(&addr_))->sin6_port = htons(port);
        }
        else {
            (reinterpret_cast<sockaddr_in*>(&addr_))->sin_port = htons(port);
        }
    }
    ...
    std::string INetAddress::Ip() {
        if (addr_.ss_family == AF_INET6) {
            ...
            const char* p = inet_ntop(AF_INET, &reinterpret_cast<sockaddr_in*>(&addr_)->sin_addr, buf, sizeof(buf));
            return p ? std::string(p) : std::string();
        }
    }
