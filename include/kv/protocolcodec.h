...
namespace adachi::network::kv::protocol {
    ...
    static constexpr uint32_t kDefaultMagic = 0x06161027;
    static constexpr uint16_t kDefaultVersion = 1;
    static constexpr uint16_t kRequest = 1;
    static constexpr uint16_t kResponse = 2;
    static constexpr uint16_t kPing = 1;
    static constexpr uint16_t kPut = 2;
    static constexpr uint16_t kGet = 3;
    static constexpr uint16_t kDel = 4;
    static constexpr uint16_t kOk = 0;
    static constexpr uint16_t kNotFound = 1;
    static constexpr uint16_t kBadRequest = 2;
    static constexpr uint16_t kInternalError = 3;
    static constexpr uint16_t kUnsupportedOpcode = 4;
    static constexpr uint16_t kBodyTooLarge = 5;
    static constexpr uint16_t kKeyTooLarge = 6;
    static constexpr uint16_t kValueTooLarge = 7;
    ...
    struct ProtocolCodec { 
        ...
        static bool WriteHostToFlow(T src, uint64_t& offset, char* store, uint64_t len) {
            if (store == nullptr) return false;
        ...
        static bool DecodePacketToKVRequest(const Packet*, KVRequest*);
        /// 如果返回false，代表传入了空指针。status字段为非kOk时也会返回true（当不存在空指针传入时）
        static bool DecodePacketToKVResponse(const Packet*, KVResponse*);
        static bool EncodeKVResponseToPacket(const KVResponse*, uint16_t opcode, uint64_t request_id, Packet*);
    };
